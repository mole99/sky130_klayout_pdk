# Taher Kourany, 27.11.22 -- Initial version of High-Vt pmos18 pcell generator (w,l,sab,gate_contact,gate_contact_num,finger_num,LmCON,RmCON,BmCON,TmCON)
...
class pchvtPmos18Generator(pya.PCellDeclarationHelper):

    """
    Description: 1.8v Pmos Pcell for Skywaters 130nm
    """
    ...
    def __init__(self):
        ...
        super(pchvtPmos18Generator, self).__init__()
        ...
        self.param("des_param", self.TypeString, "Description", default= "SkyWater 130nm High-Vt PMOS18 Pcell", readonly = True)
    ...
    def display_text_impl(self):
        ...
        return "pchvtPmos18 (w=%.4gum,l=%.4gum)" % (self.w,self.l)
    ...
    def _hvtmos18FingerTrans(self, cell, well, w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON):

      instpcMos18Finger = pcMos18FingerGenerator()
      hvtmos18 = instpcMos18Finger._MOS18Finger(self.layout,self.cell,well,w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON) 

    def _hvtpgringTrans(self, cell, well, subwell, w, l, sab, gate_contact_num, finger_num, subring, typ, grCovmCON):
      ...
      gring = instpcGRing._GRing(self.layout, self.cell, well, False, False, wgring, lgring, hgring, self.LmCON, self.RmCON, self.BmCON, self.TmCON, grCovmCON)

      if subring:
        if grCovmCON > 99.999:
          wsubgring_licon = licon_size+2*max(li_enc_licon_2,0)
          wsubgring_mcon = mcon_size+2*met_mcon_enc_2
          #wsubgring_via = via_size+2*max(met1_via_enc_2,met2_via_enc_2)
        else:
          wsubgring_licon = licon_size
          wsubgring_mcon = mcon_size+2*met_mcon_enc_1
          #wsubgring_via = via_size
        
        wsubgring = max(wsubgring_licon, wsubgring_mcon)
        ...
        subgring = instpcGRing._GRing(self.layout, self.cell, subwell, False, False, wsubgring, lsubgring, hsubgring, self.LmCON, self.RmCON, self.BmCON, self.TmCON, grCovmCON)

      if typ != "none":
        if typ == "hvtp":
          # draw hvtp high-vt mos to vt adjust implants
          lhvtp = lgring
          hhvtp = hgring
          l_hvtp = self.layout.layer(hvtp_lay_num,hvtp_lay_dt)
          self.cell.shapes(l_hvtp).insert(pya.DBox(-lhvtp/2.0, -hhvtp/2.0, lhvtp/2.0, hhvtp/2.0))
      
    def _hvtPmos18(self, w, l, sab, gate_contact, gate_contact_num, finger_num, subring, grCovmCON, sdCovmCON):
      # well input to _stdmos18FingerTrans
      self.wellgring = "N+Tap in DNW"
      
      # subring implant input to _stdmos18FingerTrans
      self.wellsubgring = "P+Tap"
      
      # diff implant input to _stdmos18FingerTrans
      self.wellmos18 = "P+S/D"
      
      self._hvtpgringTrans(self.cell, self.wellgring, self.wellsubgring, w, l, sab, gate_contact_num, finger_num, subring,"hvtp", grCovmCON)
      self._hvtmos18FingerTrans(self.cell, self.wellmos18, w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON)

    def produce_impl(self):

      # call GRing sub fucntion (__pcPmos18)
      hvtpmos18 = self._hvtPmos18(self.w, self.l, self.sab, self.gate_contact,self.gate_contact_num, self.finger_num, self.subring,self.grCovmCON,self.sdCovmCON)
