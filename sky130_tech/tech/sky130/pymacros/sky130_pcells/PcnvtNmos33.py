# Taher Kourany, 28.11.22 -- Initial version of nmos33 pcell generator (w,l,sab,gate_contact,gate_contact_num,finger_num,grCovmCON,sdCovmCON)
...
class pcnvtNmos33Generator(pya.PCellDeclarationHelper):

    """
    Description: 3.3v Native Nmos Pcell for Skywaters 130nm
    """
    ...
    def __init__(self):
        ...
        super(pcnvtNmos33Generator, self).__init__()
        ...
        self.param("des_param", self.TypeString, "Description", default= "SkyWater 130nm Native NMOS33 Pcell", readonly = True)
    ...
    def display_text_impl(self):
        ...
        return "pcnvtNmos33 (w=%.4gum,l=%.4gum)" % (self.w,self.l)
    ...
    def coerce_parameters_impl(self):
      ...
      # min length for 3.3v native nmos = 0.5um
      l_min = 0.5 
      ...
      if self.l < max(l_min,li_spc,poly_size):
        self.l = max(l_min,li_spc,poly_size)
    ...
    def _nvtmos33FingerTrans(self, cell, well, w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON):

      instpcMos33Finger = pcMos18FingerGenerator()
      stdmos33 = instpcMos33Finger._MOS18Finger(self.layout,self.cell,well,w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON)

    def _nvtn33gring(self, cell, w, l, sab, gate_contact_num, finger_num,typ,grCovmCON):
      ...
      well = "N+Tap in DNW"
      gr_dnwell = instpcGRing._GRing(self.layout, self.cell, well, False, True, wgring2, lgring2, hgring2, self.LmCON, self.RmCON, self.BmCON, self.TmCON, grCovmCON)
      ...
      if typ != "none":
        if typ == "03v3_nvt":
          ...
          l_areaid_le = self.layout.layer(areaid_le_lay_num,areaid_le_lay_dt)
          self.cell.shapes(l_lvtn).insert(pya.DBox(-llvtn/2.0, -hlvtn/2.0, llvtn/2.0, hlvtn/2.0))
          self.cell.shapes(l_areaid_le).insert(pya.DBox(-llvtn/2.0, -hlvtn/2.0, llvtn/2.0, hlvtn/2.0))
    ...
    def _nvtNmos33(self, w, l, sab, gate_contact, gate_contact_num, finger_num, grCovmCON, sdCovmCON):

      # diff implant input to _nvtmos33FingerTrans
      self.wellmos33 = "N+S/D"
      
      # instantiate guard ring
      self._nvtn33gring(self.cell, w, l, sab, gate_contact_num, finger_num,"03v3_nvt", grCovmCON)
      
      self._nvtmos33FingerTrans(self.cell, self.wellmos33, w, l, sab, gate_contact, gate_contact_num, finger_num, sdCovmCON)

    def produce_impl(self):

      # call GRing sub fucntion (__pcnvtNmos33)
      nvtnmos33 = self._nvtNmos33(self.w, self.l,self.sab,self.gate_contact,self.gate_contact_num,self.finger_num,self.grCovmCON,self.sdCovmCON)
